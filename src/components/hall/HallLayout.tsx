...
export function HallLayout({ show, onSeatSelectAction }: HallLayoutProps) {
  ...
  const handleSeatClick = (seat: Seat) => {
    ...
    setSelectedSeats(prevSeats => {
      // Prüfen ob der Sitz bereits ausgewählt ist
      const isSelected = prevSeats.some(s => s.id === seat.id);
      
      if (isSelected) {
        // Wenn ausgewählt, entferne ihn
        const newSeats = prevSeats.filter(s => s.id !== seat.id);
        onSeatSelectAction?.(newSeats);
        return newSeats;
      } else {
        // Wenn nicht ausgewählt, füge ihn hinzu
        const newSeats = [...prevSeats, seat];
        onSeatSelectAction?.(newSeats);
        return newSeats;
      }
    });
  ...
  const getSeatStyle = (seat: Seat) => {
    const baseStyle = "w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded flex items-center justify-center m-0.5 transition-all duration-200"
    if (seat.seat_status === 'BOOKED') {
      return `${baseStyle} bg-gray-600 cursor-not-allowed`
    }
    
    const category = categories.find(cat => cat.id === seat.category_id);
    if (category) {
      return `${baseStyle} ${category.id === 1 ? 'bg-emerald-600 hover:bg-emerald-700' :
        category.id === 2 ? 'bg-blue-600 hover:bg-blue-700' : 'bg-purple-600 hover:bg-purple-700'}`
    }
                        ...
                        return (
                          <button
                            ...
                            className={`
                              ...
                              ${seatData.seat_status === 'BOOKED' 
                                ? 'bg-gray-600 cursor-not-allowed' 
                                : seatData.category_id === 1 
                                  ? 'bg-emerald-600 hover:bg-emerald-700'
                                  : seatData.category_id === 2 
                          ...
                          >
                            {seatData.seat_status === 'BOOKED' ? (
                              <Ban className="w-1/2 h-1/2 text-white" />
                            ) : (
                              <>
                                {seatData.category_id === 1 && <Square className="w-1/2 h-1/2 text-white" />}
                                {seatData.category_id === 2 && <Armchair className="w-1/2 h-1/2 text-white" />}
                                {seatData.category_id === 3 && <Crown className="w-1/2 h-1/2 text-white" />}
                              </>
                            )}
                          </button>
                        );
